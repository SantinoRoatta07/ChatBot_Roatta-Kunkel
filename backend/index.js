...
async function procesarPreguntaClinica(pregunta) {
  ...
  try {
    ...
    if (texto.includes('hola') || texto.includes('buenos días') || texto.includes('buenas tardes') || texto.includes('buenas noches') || texto.includes('buenos dias')) {
      return '¡Hola! Soy el asistente de la clínica. Puedes consultar sobre horarios, seguros o especialidades.';
    }

    if (texto.includes('gracias') || texto.includes('chau') || texto.includes('adios') || texto.includes('hasta luego')) {
      return '¡Que tengas un buen día. Estamos para ayudarte si tienes más consultas.';
    }
...
// 7. Escuchar conexiones de Socket.IO
io.on('connection', (socket) => {
  console.log('Usuario conectado:');
  socket.removeAllListeners('pregunta'); // Tu código para limpiar listeners
